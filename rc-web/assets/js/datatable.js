  ...
  $.ajax({
    ...
    success: function (data) {
      try {
        ...
        let columnHeaders = allColumnHeaders
          .slice(allColumnHeaders.indexOf("address"))
          .filter((col) => col !== "availability");
        ...
        let originalData = $.extend(true, [], tableData);
        ...
        dataTable.columns().every(function () {
          ...
          if (columnsWithDropdownFilter.includes(column.header().textContent)) {
            ...
            columnsWithNumericFilter.includes(column.header().textContent)
          ) {
            ...
            $(
              ...
              .on("keyup change", function () {
                let minVal = parseFloat($(this).val()) || 0;
                let maxVal =
                  parseFloat(header.find(".max").val()) || Number.MAX_VALUE;

                // Apply the filter against the original data
                column
                  .search(
                    function (value, index, originalData) {
            ...
            $(
              ...
              .appendTo(header)
              .on("keyup change", function () {
                let minVal = parseFloat(header.find(".min").val()) || 0;
                let maxVal = parseFloat($(this).val()) || Number.MAX_VALUE;
