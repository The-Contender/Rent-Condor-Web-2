(function () {
  ...
  function initMap(coords) {
    ...
    currentPopup = popup;
    map.on("mouseenter", "interactive-points-layer", (e) => {
    ...
    map.on("click", "interactive-points-layer", (e) => {
      // Change the cursor style as a UI indicator.
      showPopup(e, map, popup);
      addPopupEventListener(overPopup, popup, map, currentPopup, popupTimeout);
    });

    // General listener for clicks on the map
    map.on("click", (e) => {
      // Check if the click is on an interactive point\
      const features = map.queryRenderedFeatures(e.point, {
        layers: ["interactive-points-layer"],
      });

      // If no features from the interactive layer were clicked, close any open popups
      if (!features.length) {
        if (currentPopup) {
          currentPopup.remove();
          currentPopup = null;
  ...
  async function addPoints(properties, map) {
    ...
    map.addLayer({
      ...
      paint: {
        "circle-radius": 40, // Larger radius for interactions
  ...
  function showPopup(e, map, popup) {
    ...
    const description = `
      ...
      <a href="${e.features[0].properties.link}" target="_blank">Listing Link</a>
    `;
  ...
  function addPopupEventListener(
    ...
    map.on("mouseleave", "interactive-points-layer", () => {
  ...
  async function fetchData() {
    try {
      const response = await fetch("https://rc-express.vercel.app/");
